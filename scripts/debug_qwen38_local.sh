...
if [[ "${1:-}" == vllm ]]; then
  ...
  exec "${VLLM_PYTHON:-$TASK_ROOT/paibench/environments/paibench_vllm_0_19_0/bin/python}" -m vllm.entrypoints.openai.api_server \
    ...
    --tensor-parallel-size 2 --max-model-len 98304 --max-num-seqs 2 \
  ...
  export REWARD_WMREWARD_DEVICE="${REWARD_WMREWARD_DEVICE:-cuda:2}"
